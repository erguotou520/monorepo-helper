...
export function getInstallDepCommand(deps: string[], {
  isDev, packageName, folderName
}: {
  isDev: boolean;
  packageName?: string;
  folderName?: string;
...
export function getRemoveDepsCommand(dep: string, {
  isDev, packageName, folderName
}: {
  isDev: boolean;
  packageName?: string;
...
export function getScriptCommand(script: string, {
  ...
  return {
    ...
    pnpm: ['pnpm', 'run', script, ...(packageName ? ['--filter', packageName] : [])],
  }[getManageTool()];
}